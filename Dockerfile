...
RUN npm install && npm install cache clean --force

COPY . .

CMD [ "/sbin/tini", "-s", "--", "node", "./bin/www" ]
# Bonus Extra Credit
# this will not have you setting up a complete image useful for local development, test, and prod
# it's just meant to get you started bin basic Dockerfile concepts and not focus too much on
